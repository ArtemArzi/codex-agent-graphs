...
- `bootstrap` — проект ещё не имеет рабочего основания Project Start. Нормализуй его под единый документационный контракт: карта, доменный контекст, техническое основание, кодовая база, качество, план и агентский контекст.
...
2. Выполни обязательный MCP-first проход. Сначала осмотри доступные `mcp__*` tools/resources и примени релевантный сервер: provider-specific MCP для его продукта, Context7 для официальной документации библиотек, research MCP для внешнего поиска, service MCP для GitHub/Linear/Notion или live-системы. Для внутренней кодовой базы по-прежнему используй локальный поиск и LSP/AST. Нативный web/browser и затем `curl` допустимы только после отсутствия или сбоя подходящего MCP. Чисто локальный проход без подходящего сервера явно фиксирует fallback, а не молча игнорирует MCP.
3. В bootstrap обязательно примени `domain-modeling` для доменного слоя и `codebase-design` для карты модулей внутри того же `work`, не превращая их в отдельные стадии. Для per-repo skill contract сначала используй `setup-matt-pocock-skills`, если он реально доступен. Если его нет, выполни внутренний fallback Project Start по [documentation-contract.md](references/documentation-contract.md), шаблону вложенного `AGENTS.md` и структурным проверкам runner; запиши `project-start:skill-contract-fallback` в `capabilities`. Отсутствие внешнего skill не блокирует корректный bootstrap. В maintenance подключай дисциплины только при изменении соответствующей роли. Остальные навыки подключай по потребности:
...
4. Обнови документы сам. Не отдавай параллельным агентам пересекающиеся документы и не запускай отдельного «куратора» по умолчанию.
...
В `capabilities` обязательно запиши `mcp:<server>` для реально вызванного сервера, например `mcp:context7`, или `mcp:fallback:<reason>` после проверки доступности. Fallback не разрешает пропустить доступный релевантный MCP. Не выполняй внешнюю запись через MCP без полномочий пользователя.
...
`complete` повторно сверяет SHA-256 артефактов и документов, переводит bootstrap в `execution`, закрывает maintenance obligation и сохраняет канонический набор в `.project-start/state.json`. После bootstrap следующая программная работа принадлежит `task-delivery`; после неё Project Start снова запускается только для документационной дельты.
