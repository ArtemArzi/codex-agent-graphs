...
- `bootstrap` — проект ещё не имеет рабочего основания Project Start. Закрой бизнес-контекст, техническое основание, качество, план и агентский контекст.
...
1. Прочитай ближайшие инструкции и существующую карту документации. Проверь `git status`; не присваивай чужие изменения.
2. Исследуй только вопросы, от которых зависит документ. Для внутренней кодовой базы сначала используй обычный поиск, LSP/AST и доступные локальные инструменты. Подключи explorer только для большой независимой области.
3. Подгружай навыки по потребности, а не полным обязательным каталогом:
   ...
   - `setup-matt-pocock-skills` — одноразово, если Sandcastle-набор ещё не настроен в проекте и его установка разрешена;
   ...
   - навыки Sandcastle — как возможный способ последующей реализации, не как вложенный оркестратор.
...
Подробные правила режима maintenance — в [references/maintenance.md](references/maintenance.md), роли и стоимость — в [references/agent-operations.md](references/agent-operations.md).
