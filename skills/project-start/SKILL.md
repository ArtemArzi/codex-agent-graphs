...
словами. Сначала скажи, что изменилось в документах или что мешает работе,
затем — влияет ли это на разработку или решение пользователя, и в конце — что
...
Если исходники или конфигурация изменились после `init`, не подменяй baseline: закрой run через `abandon` и выполни свежий `init`. До успешного replacement run Project Start остаётся `restart-required`. Чисто технический restart без дрейфа канонических документов и без обязательной повторной проверки не блокирует независимую Task Delivery; незакрытая obligation, document drift, verifier requirement и существенное решение по-прежнему блокируют её fail-closed. Неразрешённое существенное решение нельзя обойти через `abandon`. Если в abandoned run уже менялись документы, свежий `init` останется заблокирован до их восстановления к исходному digest; сохрани нужную дельту отдельно и примени её уже в новом проходе.
