...
словами. Сначала скажи, какая проблема найдена или почему менять ничего не
нужно, затем — что это значит для проекта, и в конце — что будет дальше. Не
показывай журнал контроллера вместо объяснения.

Обязательный порядок: результат → влияние → следующий шаг.

Слова `controller`, `root`, `worker`, `candidate`, `receipt`, `digest`,
`checkpoint`, `gate`, `verification` и `recovery` используй только когда точное
служебное имя нужно пользователю для действия или проверки; при первом упоминании
сразу объясни его обычными словами. Хеши, точные имена артефактов и
