...
| `standard` | self | root | `task_result_reviewer` | 1 |
| `complex` | `task_plan_reviewer` | root, опционально до 2 workers | `task_result_reviewer` | 2 |
| `critical` | `task_plan_reviewer` | root + `task_risk_reviewer` | `task_result_reviewer` | 3 |
...
- `task_worker` — реализация одного независимого slice по immutable packet. Обычно 0, максимум 2 packet/worker receipts; root принимает реальный diff по [implementation-slices.md](implementation-slices.md).
