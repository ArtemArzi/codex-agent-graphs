...
- `implementation.strategy` равен `root-only` или зарегистрированной `delegated-sequential`. Root-only является adaptive fast path для любого профиля; явный запрос `delegated-sequential` запрещает root-only. В graph `3.4+` каждый успешный worker receipt обязан получить immutable root acceptance до следующего packet или final work; каждый принятый slice содержит точные `packet_sha256`, `receipt_sha256` и `acceptance_sha256`, а `implementation.changed_paths` точно равен объединению `verified_changed_paths` из всех acceptance без скрытого root-only integration exception. Canonical root acceptance хранится отдельно и проверяется runner по [implementation-slices.md](implementation-slices.md).
